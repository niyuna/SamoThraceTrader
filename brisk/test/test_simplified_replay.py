"""
测试简化后的replay配置
验证不需要replay_symbols参数，通过subscribe方法控制
"""
...
def test_simplified_replay():
    """测试简化后的replay配置"""
    print("=== 测试简化后的Replay配置 ===")
    ...
    def on_tick_with_logging(event):
        ...
        if received_ticks[tick.symbol] <= 3:  # 只打印前3个tick
    ...
    try:
        ...
        while True:
            ...
            if tick_count % 10 == 0:
                ...
                # 检查是否收到了预期的股票tick
                expected_symbols = strategy.market_cap_eligible
                received_symbols = set(received_ticks.keys())
                missing_symbols = expected_symbols - received_symbols
                if missing_symbols:
                    print(f"⚠️  注意：未收到预期股票的tick: {missing_symbols}")
                else:
                    print("✅ 正确：收到了所有预期股票的tick")
