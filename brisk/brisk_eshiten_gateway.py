...
from common import tachibana_api as eshiten_api
...
class BriskEshitenGateway(BaseGateway):
    """
    Brisk Gateway for Japanese Stock Market
    """

    default_name: str = "BRISK_ESHITEN"
    ...
    def __init__(self, event_engine: EventEngine, gateway_name: str) -> None:
        ...
        if hasattr(eshiten_api.e_shiten_api_env['class_cust_property'], 'sUrlRequest'):
            ...
            self.write_log("Eshiten API URL is None")
            return
    ...
    # TODO: need update to eshiten api: done
    def send_order(self, req: OrderRequest) -> str:
        self.write_log(f"send_order: {req}")
        order_id, order_date = None, None

        if req.offset == Offset.OPEN:
            # note: eshiten api will return a tuple instead of a string here
            order_res = eshiten_api.send_general_init_trading_order_eshiten(req.symbol, Direction_to_TradingSide[req.direction], req.price, req.volume, FrontOrderType.MARKET if not req.price else FrontOrderType.LIMIT)
            if order_res:
                order_id, order_date = order_res
        elif req.offset == Offset.CLOSE:
            order_res = eshiten_api.send_close_position_order_eshiten(req.symbol, Direction_to_TradingSide[req.direction], req.price, req.volume)
            if order_res:
                order_id, order_date = order_res
        else:
    ...
    def cancel_order(self, req: CancelRequest) -> None:
        cancel_result = eshiten_api.cancel_order_eshiten(req.orderid, self.formatted_date)
    ...
    def _poll_orders(self):
        """轮询订单状态"""
        # eshiten can only return full day orders
        orders = eshiten_api.get_order_list_eshiten(self.formatted_date).get('aOrderList', [])
        ...
        for broker_order in orders:
            try:
                ...
                # else:
                #     self.write_log(f"WARNING: eshiten query order API: 订单状态未变化: {orderid} {old_order.status if old_order else 'NEW'} -> {order.status}")
            except Exception as e:
    ...
    def _convert_broker_order_to_vnpy(self, broker_order: Dict) -> OrderData:
        ...
        volume = float(broker_order["sOrderOrderSuryou"])
        traded = float(broker_order["sOrderYakuzyouSuryo"])

        state = Status.SUBMITTING
        if broker_order["sOrderStatusCode"] == '9':
            state = Status.PARTTRADED
        elif broker_order["sOrderStatusCode"] == '10':
            state = Status.ALLTRADED
        elif broker_order["sOrderStatusCode"] in ['7', '12']:
            state = Status.CANCELLED
        elif broker_order["sOrderStatusCode"] in ['1', '3', '4', '6']:
            state = Status.NOTTRADED
        elif broker_order["sOrderStatusCode"] in ['0', '13']:
            state = Status.SUBMITTING
        elif broker_order["sOrderStatusCode"] in ['2', '14']:
        ...
        recv_time = broker_order["sOrderOrderDateTime"]
        # recv_time is in JST
        dt = datetime.strptime(recv_time, "%Y%m%d%H%M%S")
        
        order = OrderData(
            gateway_name=self.gateway_name,
            symbol=broker_order["sOrderIssueCode"],
            exchange=Exchange.TSE,
            orderid=broker_order["sOrderOrderNumber"],
            type=OrderType.LIMIT if float(broker_order['sOrderOrderPrice']) > 0 else OrderType.MARKET,
            direction=direction_mapping.get(broker_order["sOrderBaibaiKubun"], Direction.LONG),
            offset=Offset.OPEN if broker_order['sGenkinSinyouKubun'] == '2' else Offset.CLOSE,  # 暂时设为NONE，后续可根据CashMargin和DelivType判断
            price=float(broker_order["sOrderOrderPrice"]),
            volume=volume,
            traded=traded,  # 累计成交量
            status=state,
            datetime=dt,
